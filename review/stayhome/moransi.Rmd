...
<!-- ```{r} -->
<!-- gdf$rand <- runif(nrow(gdf)) -->
<!-- ggplot() +  -->
<!--   geom_sf(data = filter(gdf,rand<0.01)) -->
<!-- ``` -->

## Stay-home

```{r}
model = readRDS("../../models/stay_home/model.rds")
```
...
<!--     geom_sf(data=gdf_res_at_t, aes(fill=residual_), size=0.125) +  -->
...
for(t_ in 7:22) {
  ...
  res_rel_tables[[as.character(t_)]] <- res_df_t %>%
    filter(!(res_rel>=q025 & res_rel<=q975)) %>% 
    left_join(df %>% 
                distinct(fips, county, state, pop)) %>% 
