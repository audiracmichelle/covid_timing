...
county_train <- read_feather("../data/county_train_.feather") %>%   # 1454 counties
  ...
  # mutate(mask = !((state == "New York") & (days_since_intrv_stayhome >= 14))) %>%
  ungroup()
length(unique(county_train$fips))


model_data = stan_input_data(county_train, type="stayhome", lag=0, use_mask=exclude_ny)
model = rstan::stan_model("../stan_models/12b_cable_bent_ar1_concave.stan")
print(paste("Compiled model:", "stayhome/12b_cable_bent_ar1_concave"))
...
saveRDS(fit, "stayhome_fitted/12_cable_bent_ar1.rds")
# now pass solution
fit2 = rstan::sampling(
  model,
  data=model_data,
