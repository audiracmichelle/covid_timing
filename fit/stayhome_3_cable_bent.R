...
county_train <- read_feather("../data/county_train_.feather") %>%   # 1454 counties
  ...
  # mutate(mask = !((state == "New York") & (days_since_intrv_stayhome >= 12))) %>%
  ungroup()
length(unique(county_train$fips))


model_data = stan_input_data(county_train, type="stayhome", lag=0)

model = rstan::stan_model("../stan_models/3b_cable_bent_concave.stan")
print(paste("Compiled model:", "stayhome/3b_cable_bent_concave"))
...
saveRDS(fit, "stayhome_fitted/cable_bent.rds")
...
# 14 experiment
saveRDS(fit2, "stayhome_fitted/cable_bent_mcmc.rds")
...
# predicted = my_posterior_predict_cable_bent(fit, county_train, type="stayhome", eval_pre=FALSE)
...
# predicted_up = my_posterior_predict_cable_bent(fit, evaldata_up, type="stayhome")
...
# predicted_down = my_posterior_predict_cable_bent(fit, evaldata_down, type="stayhome")
