...
model = rstan::stan_model("../3b_cable_bent_concave.stan")
...
saveRDS(fit, "fitted/cable_bent.rds")
...
# 14 experiment
saveRDS(fit2, "fitted/cable_bent_mcmc.rds")

# fit = read_rds("fitted/cable_bent.rds")
...
predicted = posterior_predict(fit, model_data, cable_bent=TRUE, states=TRUE)
...
predicted_up = posterior_predict(fit, model_data, cable_bent=TRUE, states=TRUE, shift_timing = 10)
log_yhat_up = apply(predicted_up$log_yhat[, ix], 2, median)

predicted_down = posterior_predict(fit, model_data, cable_bent=TRUE, states=TRUE, shift_timing = -10)
