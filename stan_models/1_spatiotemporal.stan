...
transformed parameters {
  ...
  vector[N] log_rate_pre_interv = offset + rand_eff_term + pre_term + state_eff_term + spatial_eff_term;
...
model {
  ...
  // for (j in 1:M) {
  //   if (autocor < 1.0) {
  //     sum(time_term[(county_brks[j] + 2):(county_brks[j + 1])]) ~ normal(0.0, ar_scale * inv(sqrt(1 - square(autocor))));
  //   } else {
  //     sum(time_term[(county_brks[j] + 2):(county_brks[j + 1])]) ~ normal(0.0, ar_scale);
  //   }
  ...
  //   time_term[ar_starts] ~ normal(0, 0.01);
  // } else {
  ...
  if (use_mask == 1)
    time_term[mask_miss] ~ normal(0, 0.001); // these ones aren't observed
